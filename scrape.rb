...
FileUtils.mv(tempfile.path, "./archcolgdata/#{title}-cover")
end
if (icon!="")
tempfile2 = Down.download(icon)
FileUtils.mv(tempfile2.path, "./archcolgdata/#{title}-icon")
end
CSV.open("archfulldata.csv", "a+") do |csv|
		csv << fullcollegedata
end
